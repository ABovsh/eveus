...
|---|---|---|
| `compact` | Заряд · потужність · час до цільового заряду · енергія сесії · вартість одним рядком, під ним стан і смужка заряду | — (лише перегляд) |
| `status` | Ті самі показники плитками: початковий → поточний заряд, блок «До цілі», заданий → фактичний струм, енергія й вартість сесії, потужність і напруга | — (лише перегляд) |
| `control` | Ті самі плитки, а на місці потужності — перемикачі | **Одноразове заряджання**, **Зупинити заряджання**, повзунок **Струм заряджання** |
| `full` | Усе з `control`, а також параметри SOC, потужність · напруга й температури | Усе з `control`, а також **Початковий SOC**, **Цільовий SOC**, **Ємність батареї**, **Корекція SOC** і перемикач **Ліміт: SOC увімкнено** |
...
- **Розширений і Базовий режим.** Картка працює в тому режимі, який обрано для
  ...
  те, що показують плитки. У Базовому режимі замість заряду картка показує
  потужність · напругу, замість блока «До цілі» — одну плитку «Сесія» з часом,
  енергією та вартістю, а четверта плитка показує температури корпусу й вилки;
  `status` завершується енергією минулого заряду, а `full` замість параметрів
  SOC додає минулий заряд, адаптивне обмеження струму й перемикач **Захист
  заземлення**. Поле картки **Режим інтеграції** (`mode: basic`) вмикає Базовий
  вигляд незалежно від режиму інтеграції; `mode: advanced` працює, лише якщо
  інтеграція в Розширеному режимі.
...
- **Смужка заряду.** У Розширеному режимі притемнена частина показує заряд на
  ...
  картка стає фіолетовою. Помаранчева позначка на повзунку **Задати** показує
...
- **Натискання.** Натискання на плитку відкриває вікно сутності. Довге
  ...
  плитки позначає показники, які його мають. **Одноразове заряджання**,
  **Зупинити заряджання**, **Ліміт: SOC увімкнено** і **Захист заземлення**
  перемикаються одним натисканням; зупинку активного заряджання картка
  перепитує в самій плитці, і друге натискання протягом чотирьох секунд його
  виконує. Повзунок надсилає значення, коли ви його відпускаєте. Кнопки
  − / + надсилають значення після короткої паузи, тож кілька натискань поспіль
  дають одну команду. Tab переходить між плитками, Enter або пробіл діє.
