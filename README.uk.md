...
Картка Eveus є частиною інтеграції: компактний рядок показників, плитки або
плитки з керуванням заряджанням і параметрами SOC. Вона працює в Базовому чи
Розширеному режимі інтеграції — у Базовому параметрів SOC немає, тож виглядів
там три, а не чотири, — показує несправності й просить підтвердження перед
зупинкою активного заряджання. Для окремих карток і графіків також є готова
панель.
...
- **У Базовому режимі вигляду `full` немає.** У Базовому режимі немає сутностей
  SOC, тож немає й рядка параметрів: редактор картки не пропонує `full`, а
  картка, якій його вже задано, показує `control`. Плитки заряду й смужки заряду
  в Базовому режимі теж немає. Картка працює в тому режимі, який обрано для
  інтеграції (**Налаштувати**); поле **Режим інтеграції** (`mode: basic`)
  перемикає її на Базовий вигляд, а `mode: advanced` працює, лише якщо
  інтеграція в Розширеному режимі.
- **Рядок стану** (`status`, `control`, `full`). Стан, смужка заряду, а праворуч —
  напруга мережі й обидва датчики температури: спершу вилка, потім корпус.
  Напруга стає помаранчевою поза межами 215–245 В і червоною нижче 205 В або вище
  253 В; температура — помаранчевою від 60 °C і червоною від 70 °C. Коли станція
  не заряджає, поряд зі станом з'являється `Not Charging Reason` — у всіх
  виглядах; стан `Error` або відсутнє заземлення додають червоний рядок.
- **Смужка заряду** (Розширений режим). Притемнена частина — заряд на початку
  сесії, кольорова — енергія, додана відтоді; рисочки позначають початковий і
  цільовий заряд, а щойно досягнуто цільового заряду, показники стають
  фіолетовими.
- **Коли станція не відповідає.** Картка притемнює плитки, пише *Немає зв'язку* й
  скільки часу минуло від останнього показника, і не реагує на керування.
  Початковий і цільовий заряд зберігаються в Home Assistant, тож на смужці
  лишається рисочка цілі, а самого заряду картка не знає.
- **Натискання.** Натискання відкриває сутність показника, довге натискання —
  налаштування за ним: **Заряд** — Початковий SOC, **До цілі** — Цільовий SOC,
  **Струм** — Струм заряджання. Зупинку активного заряджання картка перепитує в
  самій плитці: друге натискання протягом чотирьох секунд його виконує.
