...
| `status` | Ті самі показники плитками: початковий → поточний заряд, блок «До цілі», заданий → фактичний струм, сесія, потужність | — (лише перегляд) |
| `control` | Заряд, блок «До цілі» й сесія, а поруч — чотири перемикачі | **OCPP**, **Безліміт**, **Один заряд**, **Стоп**, повзунок **Струм** |
| `full` | Усе з `control`, а також параметри SOC | Усе з `control`, а також **Початковий SOC**, **Цільовий SOC**, **Ємність батареї**, **Корекція SOC** |

- **Розширений і Базовий режим.** Картка працює в тому режимі, який обрано для
  інтеграції (**Налаштувати**). Будова в обох режимах однакова — різниться лише
  те, що показують плитки. У Базовому режимі `status` показує потужність, сесію
  та струм; у `control` сесія стоїть між двома парами перемикачів; `full`
  замість параметрів SOC додає енергію й вартість від скидання лічильника,
  енергію минулого заряду та перемикач **Захист заземлення**. Поле картки
  **Режим інтеграції** (`mode: basic`) вмикає Базовий вигляд незалежно від
  режиму інтеграції; `mode: advanced` працює, лише якщо інтеграція в
  Розширеному режимі.
- **До цілі.** Під час заряджання плитка показує цільовий заряд і час, що
  лишився, а в простої — різницю між поточним і цільовим зарядом. Нижче йдуть
  енергія й вартість, яких ще бракує, і час завершення. Показник, якого станція
  зараз дати не може, картка не виводить замість того, щоб писати `--`.
- **Напруга й температура.** У виглядах `status`, `control` і `full` праворуч
  у рядку стану картка показує напругу мережі й обидва датчики — спершу вилку,
  потім корпус, — тож вони не займають окремого місця; у `compact` напруга
  стоїть у тому самому рядку, що й решта значень. Напруга стає помаранчевою
  поза межами 215–245 В і червоною нижче 205 В або вище 253 В; температура —
  помаранчевою від 60 °C і червоною від 70 °C. Натискання відкриває потрібний
  датчик.
- **Повзунок струму.** Повзунок задає струм заряджання, а в його кінці стоїть
  потужність, яку станція видає зараз, тож жодне з цих значень не дублюється
  плиткою. Помаранчева позначка на ньому показує адаптивне обмеження струму,
  коли воно нижче за максимум повзунка.
- **Читабельність.** Відцентровані підписи й значення мають більший шрифт і за
  потреби переносяться. `compact` вміщає всі значення в один рядок: на вузькій
  картці шрифт стає на пункт меншим, а не переноситься. Під час заряджання
  картка яскравіше пульсує та показує рухомі відблиски; анімація враховує
  налаштування зменшення руху на пристрої.
- **Смужка заряду.** У Розширеному режимі притемнена частина показує заряд на
  початку сесії, а анімована кольорова — енергію, додану відтоді; рисочки
  позначають початковий і цільовий заряд. Щойно досягнуто цільового заряду,
  картка стає фіолетовою.
- **Чому станція не заряджає.** Під станом картка пише `Not Charging Reason` —
  очікування авто, очікування розкладу, керування через OCPP, заряд завершено
  тощо. Рядок з'являється лише тоді, коли є що показати.
- **Коли станція не відповідає.** Початковий і цільовий заряд зберігаються в
  Home Assistant, тож вони лишаються навіть тоді, коли решта показників зникла.
  Тому картка притемнює плитки, пише *Немає зв'язку* й скільки часу минуло від
  останнього показника, лишає на смужці тільки рисочку цілі та не реагує на
  керування — замість того, щоб малювати заряд, якого немає.
- **Натискання.** Натискання на плитку відкриває вікно сутності. Довге
  ...
  SOC, **До цілі** — Цільовий SOC, **Струм** — Струм заряджання; крапка в кутку
  плитки позначає показники, які його мають. **OCPP**, **Безліміт**, **Один
  заряд**, **Стоп** і **Захист заземлення** перемикаються одним натисканням;
  зупинку активного заряджання картка перепитує в самій плитці, і друге
  натискання протягом чотирьох секунд його виконує. Повзунок надсилає значення,
  коли ви його відпускаєте. Кнопки − / + надсилають значення після короткої
  паузи, тож кілька натискань поспіль дають одну команду. Tab переходить між
  плитками, Enter або пробіл діє.
- **Несправності.** Якщо станція в стані `Error` або немає заземлення, червоний
  рядок з'являється в усіх виглядах. Показники, які картка тримає з часу до
  втрати зв'язку, тривоги не піднімають.
