...
*Картка Eveus для панелі. Ви додаєте одну картку й обираєте для неї один із чотирьох виглядів: `compact`, `status`, `control` або `full`. Ліворуч — Розширений режим, праворуч — Базовий. [Як додати картку](#картка-eveus).*

Інтеграція для зарядних станцій Eveus: керування заряджанням, електричні вимірювання, облік енергії та вартості, оцінка рівня заряду (SOC) батареї авто, розклади, сповіщення безпеки та сутності для автоматизацій. Інтеграція звертається до HTTP API станції напряму в локальній мережі, тому працює й без інтернету. Усі дані та налаштування станції стають звичайними сутностями Home Assistant.
...
Усе, що вимірює станція, оновлюється під час кожного опитування:
...
Інтеграція показує **Енергію сесії**, **Загальну енергію** і два лічильники A та
B, які можна скидати окремо. Для сесії й кожного лічильника є сенсор вартості.
...
У **Розширеному** режимі інтеграція сама створює сутності SOC — помічники вручну
створювати не треба:

- **Рівень заряду** (%) і **Заряд батареї** (кВт·год) — оцінка заряду батареї
  під час заряджання;
- **Час до цільового заряду** і **Час завершення заряджання** — коли заряджання
  завершиться;
- **Енергія до цільового заряду** і **Вартість до цільового заряду** — скільки
  енергії ще потрібно передати в авто і скільки вона коштуватиме;
...
Станція може зупинити сесію автоматично. Кожен ліміт, який підтримує станція,
задається з Home Assistant і має власний перемикач:

- **Ліміт: Час**, **Ліміт: Енергія** і **Ліміт: Вартість** — зупинка за
  тривалістю сесії, переданими кВт·год або вартістю сесії;
- **Ліміт: SOC увімкнено** (Розширений режим) — зупиняє заряджання, коли заряд
  авто досягає **Цільового SOC**;
...
Адаптивний режим захищає слабку електромережу будинку: коли напруга просідає,
...
- **Підтвердження команд** — інтеграція перевіряє результат кожної команди й
...
Одне випадкове хибне вимірювання не створює сповіщення: інтеграція підтверджує
небезпечний стан кількома вимірюваннями поспіль, а знімає сповіщення лише після
стабільного відновлення. Повний перелік — у розділі
[Сповіщення безпеки](#-сповіщення-безпеки).
...
заряд авто досягає **Цільового SOC**, інтеграція надсилає звичайну команду
...
| `sensor.eveus_ev_charger_last_session_duration` | — | **Тривалість останньої сесії** — тривалість останньої завершеної сесії |
...
Розширений режим створює чотири сутності `number` для параметрів SOC. Старі
помічники `input_number.ev_*` більше не зчитуються.
...
розрахунок для поточної сесії продовжується без додаткових опорних значень.
...
перед заряджанням. Більше нічого не змінюється: **Рівень заряду**, **Заряд
батареї** і сенсори цільового заряду та часу інтеграція, як і раніше,
розраховує сама — з **Початкового SOC** і лічильника енергії станції.
...
`sensor` із класом пристрою `battery` і одиницею `%`: список показує тільки
такі, тож якщо вашого сенсора там немає, у нього інший клас пристрою. Поле є
лише в Розширеному режимі. Якщо залишити його порожнім (типово), нічого не
зміниться.
...
**Початковий SOC** не змінюється, і всі показники SOC лишаються правильними.
Нову сесію починає лише від'єднання авто.
...
Застаріле значення **Початкового SOC** спотворює всі показники SOC без жодної
...
Готова вкладка панелі типу **Sections** з **усіма сутностями Eveus** лежить у
...
> і файл видасть помилку. Його потрібно додати в текстовий редактор панелі, до
...
місяці. Вартість інтеграція рахує сама — див.
...
| --- | --- | --- |
| Сповіщення про сесію заряджання | Надсилає сповіщення про початок і завершення сесії з енергією, вартістю та тривалістю — через обрану вами дію | [`notify_session.yaml`](https://github.com/ABovsh/eveus/blob/main/blueprints/automation/eveus/notify_session.yaml) |
| Зупинка заряджання при низькому заряді домашньої батареї | Коли заряд батареї інвертора падає нижче порогу, зупиняє заряджання командою самої станції **Зупинити заряджання**, а не вимкненням живлення. Працює повністю локально | [`stop_on_low_house_battery.yaml`](https://github.com/ABovsh/eveus/blob/main/blueprints/automation/eveus/stop_on_low_house_battery.yaml) |
...
| Немає сутностей SOC | Через **Налаштувати** оберіть **Розширений (SOC + час до цільового рівня)**; якщо режим щойно змінено, перезавантажте інтеграцію |
...
Перевірте по черзі — ці три причини трапляються значно частіше, ніж справжня
несправність:
...
Робоча станція створює приблизно 85 сутностей. Якщо ви бачите лише одну сутність
...
Інтеграція зберігає лише дані підключення до станції, потрібні Home Assistant.
