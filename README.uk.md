...
Картка є частиною інтеграції. Оберіть **Вигляд** для кожної картки,
яку додаєте на панель:

### Вигляди

| Вигляд | Для чого |
| --- | --- |
| `compact` | Показники одним рядком; лише перегляд |
| `status` | Показники плитками; лише перегляд |
...
- **Попередження:** напруга помаранчева поза 215–245 В, червона — нижче 205 В або вище 253 В; температура — помаранчева від 60 °C, червона від 70 °C. Картка показує несправності, відсутність заземлення й причини зупинки заряджання.
- **Без зв'язку:** показує, скільки часу минуло від останніх даних, і блокує керування.
- **Керування:** натискання на показник відкриває сутність. Довге натискання на **Заряд**, **До цілі** або **Струм** відкриває відповідне налаштування. Для зупинки активного заряджання натисніть повторно протягом чотирьох секунд. Збільшення **Струму** теж потребує підтвердження: натисніть на нове значення протягом чотирьох секунд, щоб застосувати його; зменшення застосовується одразу.
...
3. Оберіть **Вигляд** (типово `control`) і натисніть **Зберегти**. Вкажіть **Станція**, якщо їх кілька; **Режим інтеграції** та **Мова** — необов'язкові.
