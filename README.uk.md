...
| `full` | Усе з `control`, а також параметри SOC. Лише в Розширеному режимі — у Базовому параметрів SOC немає, тож редактор цього вигляду там не пропонує | Усе з `control`, а також **Початковий SOC**, **Цільовий SOC**, **Ємність батареї**, **Корекція SOC** |
...
- **Розширений і Базовий режим.** Картка працює в тому режимі, який обрано для
  ...
  та струм, а в `control` сесія стоїть між двома парами перемикачів. Поле картки
...
- **Натискання.** Натискання на плитку відкриває вікно сутності. Довге
  ...
  заряд** і **Стоп** перемикаються одним натисканням;
