...
- **Напруга й температура в кожному вигляді.** Праворуч у рядку стану картка
  показує напругу мережі й обидва датчики — спершу вилку, потім корпус, — тож
  вони не займають окремого місця. Напруга стає помаранчевою поза межами
  215–245 В і червоною нижче 205 В або вище 253 В; температура — помаранчевою
  від 60 °C і червоною від 70 °C. Натискання відкриває потрібний датчик.
...
- **Читабельність.** Відцентровані підписи й значення мають більший шрифт і за
  потреби переносяться. `compact` тримає всі показники в одному рядку: на
  вузькій картці шрифт трохи зменшується, а не переноситься. Під час заряджання
