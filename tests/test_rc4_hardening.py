...
class _Updater:
    def __init__(self, data):
        self.data = data
        self.available = True
        self.connection_quality = {"success_rate": 100.0}
...
def test_counter_cost_rejects_negative():
    assert get_counter_a_cost(_Updater({"IEM1_money": -1.0}), None) is None
    assert get_counter_b_cost(_Updater({"IEM2_money": -0.01}), None) is None
    assert get_counter_a_cost(_Updater({"IEM1_money": 12.5}), None) == 12.5


# F06 — tariff rate negative rejected (raw, before /100)
def test_tariff_rate_rejects_negative():
    assert get_primary_rate_cost(_Updater({"tarif": -100}), None) is None
    assert get_rate2_cost(_Updater({"tarifAValue": -50}), None) is None
    assert get_rate3_cost(_Updater({"tarifBValue": -10}), None) is None
    assert get_primary_rate_cost(_Updater({"tarif": 450}), None) == 4.5


# F07 — adaptive current/voltage negative rejected
def test_adaptive_metrics_reject_negative():
    assert get_adaptive_current(_Updater({"aiModecurrent": -1}), None) is None
    assert get_adaptive_voltage(_Updater({"aiVoltage": -5}), None) is None
    assert get_adaptive_current(_Updater({"aiModecurrent": 10}), None) == 10


# F08 — session_cost MONETARY+UAH+also rejects negative
...
def test_session_cost_rejects_negative():
    assert get_session_cost(_Updater({"sessionMoney": -0.5}), None) is None
    assert get_session_cost(_Updater({"sessionMoney": 7.2}), None) == 7.2
...
async def test_repair_flow_blocks_unique_id_collision():
    ...
    other = MagicMock(entry_id="other", unique_id="192.168.1.50")
    ...
    with patch(
        ...
        result = await flow.async_step_confirm({"host": "192.168.1.50"})
