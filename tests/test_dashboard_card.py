...
def test_card_ships_in_the_package_with_picker_entry_and_editor():
    source = CARD.read_text(encoding="utf-8")
    assert "customElements.define('eveus-card', EveusCard)" in source
    assert "customElements.define('eveus-card-editor', EveusCardEditor)" in source
    assert "window.customCards" in source
    assert "getConfigElement" in source
    # Dashboards saved with the four-layout card keep opening.
    assert "const LEGACY_LAYOUTS = {" in source
...
def test_card_behaviour_suite_passes_in_node():
    """The card's behaviour lives in tests/card/: rendering, taps, confirmations, the
    editor, both languages and the saved-layout mapping, executed against the real file."""
    ...
    suite = Path(__file__).resolve().parent / "card" / "eveus-card.test.cjs"
    result = subprocess.run([node, "--test", str(suite)], capture_output=True, text=True, timeout=120, check=False)
