...
|---|---|---|
| `compact` | SOC · power · time to target · session energy · cost in one row, with the state and the SOC bar below | — (read-only) |
| `status` | The same readings as tiles: initial → current SOC, the to-target block, set → actual current, session energy and cost, power and voltage | — (read-only) |
| `control` | The same tiles, with the toggles where `status` shows power | **One Charge**, **Stop Charging**, **Charging Current** slider |
| `full` | Everything in `control`, plus the SOC settings, power · voltage and the temperatures | Everything in `control`, plus **Initial SOC**, **Target SOC**, **Battery Capacity**, **SOC Correction** and the **Limit: SOC enabled** switch |

- **Advanced and Basic mode.** The card uses the mode chosen for the integration (**Configure**). Both modes have the same shape; only what the tiles carry changes. In Basic mode the SOC tile becomes power · voltage, the to-target block becomes one Session tile with session time, energy and cost, and the fourth tile shows the box and plug temperatures; `status` ends with the last charge's energy, and `full` adds the last charge, the adaptive current limit and the **Ground protection** switch instead of the SOC settings. The card's **Integration mode** field (`mode: basic`) switches the card to its Basic view regardless of the integration's mode; `mode: advanced` needs the integration in Advanced mode.
...
- **SOC bar.** In Advanced mode the dimmer part shows the charge present at the start of the session, while the animated coloured part shows energy added since then; ticks mark the Initial and the Target SOC. The card turns violet once the Target SOC is reached. An orange mark on the **Set** slider shows the adaptive current limit when it is below the slider's maximum.
- **Why it is not charging.** Under the state the card prints the `Not Charging Reason` — Waiting for Car, Waiting for Schedule, Controlled by OCPP, Charge Complete and the rest.
