...
| `full` | Everything in `control`, plus the SOC settings | Everything in `control`, plus **Initial SOC**, **Target SOC**, **Battery Capacity**, **SOC Correction** |

- **Advanced and Basic mode.** The card uses the mode chosen for the integration (**Configure**). Both modes have the same shape; only what the tiles carry changes. In Basic mode `status` shows power, session and current; `control` keeps the session between the two pairs of switches; `full` adds the total energy and cost since the counter was reset, the last charge's energy and the **Ground protection** switch instead of the SOC settings. The card's **Integration mode** field (`mode: basic`) switches the card to its Basic view regardless of the integration's mode; `mode: advanced` needs the integration in Advanced mode.
...
- **Voltage and temperature, in every layout.** The state line carries the mains voltage and both probes — plug first, then box — at its right edge, so they cost no extra height. Voltage turns amber outside 215–245 V and red below 205 V or above 253 V; a temperature turns amber from 60 °C and red from 70 °C. Tapping one opens that sensor.
