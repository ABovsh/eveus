...
Firmware 1.x (EnergyStar V-series) also sets up and works, with some fields degraded: the firmware version is read from the charger's boot info instead of the usual field, and if the State sensor reports a value the integration doesn't recognize, it shows `Unknown (<value>)` rather than a named state. Fields the firmware doesn't report at all (such as serial number or OCPP status) stay unavailable rather than showing stale or wrong data. Fast polling while charging is inferred from measured power and current on this firmware, since a dedicated status flag isn't available.
