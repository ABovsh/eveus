...
### ⚠️ Breaking
- **`sensor.eveus_ev_charger_system_time` replaced by `sensor.eveus_ev_charger_time_drift`.** The old System Time sensor showed the charger's ticking clock, which forced Home Assistant's recorder to store a new database row on every poll — about 3,000 rows per day of data nobody graphs. The new **Time Drift** diagnostic sensor shows what actually matters: how many seconds the charger's clock is off from Home Assistant (`0 s` = in sync, negative = behind). It only changes state when the clock genuinely drifts or gets synced, so database writes drop to a handful per month, and you can automate on it (e.g. drift beyond a minute → press **Sync Time**). The old entity is removed from the registry automatically on update — no manual cleanup needed. If you used it in a dashboard, replace the card with `sensor.eveus_ev_charger_time_drift` — the bundled dashboard YAMLs are updated.
