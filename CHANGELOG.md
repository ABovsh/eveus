...
### 🐛 Fixed
- **A schedule current limit set below 7 A on the charger now displays** instead of reading unknown. The firmware stores and reports a sub-7 A schedule setpoint verbatim (verified on hardware) — the same behavior the main Charging Current already handles; Home Assistant writes still floor at 7 A.
- **The Last Session sensors no longer latch a corrupt reading.** A glitched value in the session's final poll (negative or impossibly large energy/cost/duration) used to be captured as the "last session" number and survive restarts; it now shows unknown for that session instead, matching the validation every live sensor already applies to the same fields.
- **Changing the integration mode (Basic/Advanced) now tells you if the reload failed** instead of closing as if everything succeeded; the saved settings are kept either way.
- **Reconfiguring or re-authenticating also reports a failed reload now** — the same honesty as the integration-mode dialog: the new connection details or credentials are saved either way, and a clear message tells you to reload the integration from its page instead of the dialog closing as a success.
- **An absurd firmware-version number from an older charger can no longer break setup.** The one-time firmware-version lookup for firmware 1.x units now ignores out-of-range values and simply keeps showing "Unknown".
- **Device triggers on multi-charger setups pick the right charger while it is still starting up.** An automation trigger attached while that charger's integration was mid-retry used to listen to charger #1's events instead of its own; the stored charger number is now used.
- A charger reply with an impossibly large current setting no longer fails setup with a generic "unexpected error" — it is treated like any other unreadable current value and setup proceeds.
...
- **Firmware 1.x chargers (MCU_SW_version 151) can now be set up and report correct charging states.** These firmwares report device `state` codes outside the known 0-7 range and re-use code 3 for active charging, so every poll was rejected and setup failed (#11). Any state 0-255 is now accepted, and the two known 1.x codes are translated to their modern equivalents (20 → Standby; 3 → Charging while power is actually flowing, with a short grace window so a momentary zero-power dip mid-session doesn't register as a finished charge), which makes the State sensor, charging detection, the fast poll cadence, session tracking, and charging events behave as on current firmware. A still-unrecognized code shows as `Unknown` on the State sensor with the numeric code preserved in its `raw_state` attribute. Modern firmware payloads are never translated. The adaptive poll interval additionally falls back to the reported power/current for any untranslated code.
- **Firmware 1.x chargers now show a real firmware version instead of "Unknown".** These firmware builds omit `verFWMain` from `/main` entirely (#11). At setup, the integration now falls back to `/init`'s `ESP_SW_version` (or `MCU_SW_version`) once and formats it as e.g. `1.51`. Modern firmware, which always reports `verFWMain`, never triggers this extra request.
