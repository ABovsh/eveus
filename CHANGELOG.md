...
### ✨ Added
- **Initial SOC can now fill itself in from your car's own battery sensor.** If another integration exposes your car's state of charge, pick it during setup or under **Configure → Car SOC sensor** (Advanced mode only) and the integration copies its reading into `number.eveus_ev_charger_initial_soc` once per plug-in — no more setting it by hand before every charge. Everything downstream is unchanged: SOC Percent, SOC Energy and all the target/ETA sensors are still calculated by the integration from Initial SOC and the charger's own energy meter. A session here is a plug-in, not a charge: the charger's energy counter runs from plug-in to unplug and keeps accumulating across pauses, manual stops, Charge Complete and recovered faults, so the anchor is taken once per plug-in and every SOC figure stays correct for the whole cycle. The reading is attempted on every poll while charging until one lands, so a car that is still asleep when a scheduled charge begins is picked up as soon as it wakes, and a Wi-Fi blip at the session start no longer costs the whole cycle; whatever energy is already on the charger's meter is subtracted, so a reading taken late anchors exactly like an immediate one. Once a reading lands, the car sensor is not consulted again until you unplug. A value you set by hand is never overwritten — including across a reload of the integration, so saving options mid-charge does not lose it — and a reading that cannot be used — an asleep car, an `unavailable` sensor, a reply without its energy figure, or a rebase that lands outside 0–100% — leaves Initial SOC exactly as it was and is reported once per plug-in in the log. If Home Assistant restarts or the entry is reloaded mid-charge, a cycle that had already been filled in keeps its value and a cycle that had not carries on trying. The field is optional and empty by default; leave it empty and nothing changes, including the log.
- **Diagnostics now include the SOC inputs and the car-sensor seeding outcome.** A downloaded diagnostics file carries Initial SOC, Target SOC, battery capacity and SOC correction, plus the configured car sensor, what it reads right now, and whether this plug-in cycle was seeded or the reason it was not — the figures a SOC bug report cannot be acted on without. All of it is percentages, a kWh capacity and an entity id; nothing identifying is added.
