...
- **Ground protection can now be controlled from Home Assistant.** The new opt-in **Ground Protection** switch (`switch.eveus_ev_charger_ground_protection`) mirrors the charger's `groundCtrl` setting. It is disabled by default in the entity registry because turning it off allows charging to continue without a detected ground connection. Missing ground and disabled protection remain separate Repairs issues, and both appear when both conditions are present.
