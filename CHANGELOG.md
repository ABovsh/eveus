...
- **Last Session sensors** — `sensor.eveus_ev_charger_last_session_energy`, `..._last_session_cost`, `..._last_session_duration`, and (Advanced mode) `..._last_session_final_soc`, populated when a session finishes and carrying `reason`/`finished_at` attributes. Values persist across restarts and while the charger is offline.
