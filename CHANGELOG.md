...
### ✨ Added
- **Initial SOC can now fill itself in from your car's own battery sensor.** If another integration exposes your car's state of charge, pick it under **Configure → Car SOC sensor** and the integration copies its reading into `number.eveus_ev_charger_initial_soc` once per plug-in, at the moment a charging session starts — no more setting it by hand before every charge. Everything downstream is unchanged: SOC Percent, SOC Energy and all the target/ETA sensors are still calculated by the integration from Initial SOC and the charger's own energy meter, and the car sensor is not consulted again for the rest of the session. When the start is observed a little late — the charger began charging between two polls, or charging resumed after completing — the energy already on the meter is subtracted so it is not counted twice. A Home Assistant restart mid-session seeds nothing and leaves Initial SOC as it was. A value you set by hand is never overwritten mid-session, and a car that is asleep, a sensor that reads `unavailable`, or a charger reply that arrives without its energy reading simply leaves Initial SOC as it was. The field is optional and empty by default; leave it empty and nothing changes.
