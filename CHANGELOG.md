...
### ⚠️ Breaking
- **Five readings no longer record long-term statistics.** `primary_rate_cost`, `rate_2_cost`, `rate_3_cost`, `leakage_current` and `leakage_current_peak` lose their `state_class`. The three rate sensors report the prices you configured in the charger (`tarif`, `tarif_2`, `tarif_3`), not a measurement, and leakage is an event the charger reports itself — above its 30 mA threshold it trips and says so, and `leakValueH` keeps the peak on the device. Between them they were writing about 1,440 statistics rows a day for values that did not move; statistics are never purged, so that cost was permanent. All five keep their state, their attributes, their history and their place on every dashboard card that shows a current value. What stops is the 5-minute and hourly aggregates: a Statistics Graph card or a long-term trend built on one of these will stop gaining new data, and history older than your recorder's retention is no longer available for them. Already-recorded statistics are left untouched. `active_rate_cost` is unaffected — it switches between the three rates by time of day, so it does vary — and Battery Voltage keeps its statistics deliberately: the charger's CR2032 clock cell drains over years, and the long trend is the only warning that it needs replacing.
...
- **A missed poll no longer blanks every reading.** One failed poll left the charger's sensors, binary sensors and controls — switches, the current and limit numbers, the selects, the schedule times — visible but empty before the integration turned them unavailable. Home Assistant records that gap as `unknown`, and anything built on those sensors — a utility meter, a statistic, a template — takes `unknown` as a real, invalid reading rather than skipping it the way it skips `unavailable`; a utility meter fed by Total Energy logged an error on every such gap. Each reading now holds until the charger answers again, and turns unavailable, never blank, once its window is up. The controls were affected worst while nothing was charging: their hold was measured from the last successful poll rather than from the first failed one, and at the slow idle rate the gap between those two moments is longer than the hold itself, so all of them blanked. Connection Quality keeps updating throughout — it is the reading that describes the outage.
- **Charging Finish Time no longer jumps back and forth between two times.** The estimate is re-worked from a fluctuating power reading on every poll, and one sitting between two five-minute marks alternated between them for a whole session — so an automation set to fire before the charge finishes was re-triggered every half minute, and a database row was written each time with nothing having changed. Charging Finish Time now holds the time it states until the estimate really moves: at least seven or eight minutes, and proportionally more on a long estimate, where the same power swing moves the answer further. It states the nearest five-minute mark rather than always rounding to the one after it, and never a time that has already passed. Time to Target SOC is now that same held time expressed as what is left of it, so the two always agree to within one rounding — they had been damped separately, which left them stating times up to 7 minutes apart on a two-hour charge and 17 on a ten-hour one, and left Time to Target stalling for seven and a half minutes and then jumping ten, never able to show 1h 55m at all. It now steps down through every five-minute mark. Below one minute it still reads `< 1m`, and in the last five minutes it still reads `5m`.
- **Session Time stops writing a row every minute while nothing is charging.** The charger counts a session from the moment the cable goes in and stops only when it comes out, so a car left plugged in after Charge Complete kept the figure ticking all night. It is now stated in five-minute steps whenever a charge is not actually running, and still to the minute while one is; it never counts backwards — not when a charge ends, and not across a restart or a reload of the integration either — and a new plug-in starts from zero as before.
...
- **Duration, energy, voltage and current controls now declare what they measure.** Limit Time, Limit Energy, the Undervoltage threshold and the schedule energy and current limits carry a device class, so Home Assistant labels and groups them like every other control of that kind.
- **Your recorder database grows about half as fast.** Measured over a week of normal use: ~50 % fewer rows — ~85 % while waiting for a charge, ~35 % during a session. Readings that had not meaningfully moved were writing a row on nearly every poll; now:
  ...
  - Estimates are stated in steps that match their real precision: Time to Target SOC and Charging Finish Time both in 5 minutes, so the two never disagree by more than one step.
...
- **Diagnostics downloads name the rule too.** The last recorded error kept saying only that a reply was rejected, so the file users are asked to attach to an issue was the one place the reason was missing.
...
- **Initial SOC can be filled in from your car's own battery sensor.** Pick a `sensor` with device class `battery` during setup or under **Configure → Car SOC sensor** (Advanced mode only) and its reading is copied into `number.eveus_ev_charger_initial_soc` once per plug-in, at the start of charging. Everything downstream is unchanged — SOC Percent, SOC Energy and the target/ETA sensors are still calculated from Initial SOC plus the charger's own energy meter.
  - A plug-in, not a charge, is what re-reads the car: pauses, manual stops, Charge Complete and recovered faults leave the charger's energy counter running, so the value is anchored once per plug-in and stays correct for the whole cycle.
  - If the car is asleep at the start of charging — normal with a departure timer — the reading is retried on every poll until one arrives. Energy already delivered is subtracted, so a late reading gives the same result as an immediate one.
  - A value set by hand is never overwritten, including across a restart or an options save mid-charge.
  - A reading that cannot be used leaves Initial SOC alone and is logged once per plug-in with the reason.
  - Optional and empty by default; with it empty nothing runs and nothing is logged.
- **Diagnostics now include a `soc` block.** Initial SOC, Target SOC, battery capacity and SOC correction, plus the configured car sensor, what it reads at download time, and whether this plug-in cycle was filled in or the reason it was not.
...
- **Last Session sensors** — the charger firmware clears its session counters when the next session starts, so the previous session's final numbers vanish as soon as you plug in again. Three new sensors capture them when a session finishes and keep them: `sensor.eveus_ev_charger_last_session_energy`, `..._cost`, `..._duration` (with `reason` and `finished_at` attributes). Values survive restarts and charger downtime.
...
- **Ground protection can now be controlled from Home Assistant.** The new **Ground Protection** switch (`switch.eveus_ev_charger_ground_protection`) mirrors the charger's `groundCtrl` setting. When it is on, the charger checks for a protective earth connection and blocks charging if ground is missing; when it is off, the charger only reports ground status and lets charging continue without a detected ground. Missing ground and disabled protection remain two independent Repairs notices.
