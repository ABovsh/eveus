...
- **Firmware 1.x chargers (MCU_SW_version 151) can now be set up.** These report a device `state` value outside the known 0-7 range (observed: 20), which was rejected outright, failing every poll (#11). Any state 0-255 is now accepted; an unrecognized value shows as `Unknown (<value>)` on the State sensor instead of one of the named states, and the adaptive poll interval falls back to the reported power/current to decide whether to use the fast charging cadence.
