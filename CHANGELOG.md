...
### ⚠️ Breaking
- **`sensor.eveus_ev_charger_system_time` replaced by `sensor.eveus_ev_charger_time_drift`** (thanks to @ababak for reporting). The old System Time sensor showed the charger's ticking clock, so Home Assistant's recorder stored a new database row on every poll — about 3,000 rows a day of data nobody graphs. The new **Time Drift** diagnostic shows the part that actually matters: how many seconds the charger's clock is off from Home Assistant's (`0 s` = in sync, negative = charger is behind).

  It stays quiet on purpose. Time Drift only records a change when the clock really drifts or gets synced, dropping database writes from thousands a day to a handful a month. To stop it flickering between nearby values — which would bring the recorder noise straight back — the reading is rounded into a tolerance band: ordinary poll-timing jitter always reads `0`, and the value only moves once the charger is off by **at least ~30 seconds** (widened from a finer step after testing showed small differences could still toggle the value). You can still automate on it — e.g. drift beyond a minute → press **Sync Time**.

  It compares wall clocks, so a wrong **Time Zone** select — or a daylight-saving change the charger's fixed offset doesn't follow — shows up as a steady one-hour drift instead of being invisible. The old entity is removed from the registry automatically on update; if you used it on a dashboard, swap the card for `sensor.eveus_ev_charger_time_drift` (the bundled dashboard YAMLs are already updated).

### 🔧 Changed
- **The device page shows both charger firmware versions again.** Your charger reports two firmware strings — its **Wi-Fi firmware** (e.g. `1PGRW001A-R3.05.5`, the same value the EVEUS / Grizzl-E app lists as *Firmware Version*) and its **charger-module firmware** (e.g. `GRM070A-R3.05.4`). Both were shown before 4.13; 4.13 dropped one and the remaining value no longer matched the app. This release brings both back, combined into the Devices page **Firmware** field as `Wi-Fi (module)` — the app-matching version first, the module firmware in parentheses, so you can read either from the charger's own web interface. It still updates on its own after an over-the-air firmware update — no reload needed.
- **The drifted-clock warning now tells you the right fix.** Two distinct Repairs messages instead of one: a drift sitting at a whole hour raises *"Time Zone appears incorrect"* and points at the **Time Zone** select (pressing Sync Time alone won't help — typical after a daylight-saving change), while any other offset points at the **Sync Time** button. If the situation changes while the warning is open — say the clock gets synced but the Time Zone is still wrong — the message switches to the matching guidance. Both reference the Time Drift sensor so you can watch the offset return to `0 s`, and the warning now compares local clocks, so a wrong Time Zone or DST mismatch is detected at all (previously it was invisible).

### 📊 Dashboard
- **Charger Clock card replaced by Time Drift** in both the English and Ukrainian dashboard YAMLs.

### 🐛 Fixed
- **Deleting the integration cleans up after itself.** Removing a charger entry now also removes every Repairs notice that belonged to it — including persistent ones like the SOC dashboard migration note and safety incidents — instead of leaving them orphaned in Settings → Repairs.
- **Connection Quality stays readable while the charger is unreachable.** The one sensor that describes connectivity no longer goes unknown/unavailable exactly when polls fail — success rate, latency, and failure attributes remain visible throughout an outage.
- **Charging estimates no longer drift during brief outages.** During the short grace window after a failed poll (up to a minute — enough to ride out a weak-network blip), *Charging Finish Time*, *Time to Target SOC*, and the energy/cost estimates keep their last computed value instead of being recomputed from stale data, which walked the finish time later on every failed poll. Past the grace window they go unavailable as before — stale estimates are never shown for a prolonged outage.
- **Session Active reads unknown during a charger error.** In the error state the firmware cannot tell whether the session survives, so the sensor no longer reports a definite "off" that could falsely trigger session-ended automations — matching how Car Connected already behaves.
- **Re-entering your password can no longer undo a settings change made at the same moment.** Completing re-authentication now updates only the credentials; a model, phase, or SOC-mode change committed while the password dialog was open is preserved.
