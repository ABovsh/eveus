...
### ⚠️ Breaking
- **`sensor.eveus_ev_charger_system_time` replaced by `sensor.eveus_ev_charger_time_drift`.** The old System Time sensor showed the charger's ticking clock, which forced Home Assistant's recorder to store a new database row on every poll — about 3,000 rows per day of data nobody graphs (thanks to @ababak for reporting it). The new **Time Drift** diagnostic sensor shows what actually matters: how many seconds the charger's local clock is off from Home Assistant's local time (`0 s` = in sync, negative = behind). It compares wall clocks, so a wrong **Time Zone** select — or a daylight-saving change the charger's fixed offset doesn't follow — shows up as a steady drift of exactly one hour instead of being invisible. It only changes state when the clock genuinely drifts or gets synced, so database writes drop to a handful per month, and you can automate on it (e.g. drift beyond a minute → press **Sync Time**). The old entity is removed from the registry automatically on update — no manual cleanup needed. If you used it in a dashboard, replace the card with `sensor.eveus_ev_charger_time_drift` — the bundled dashboard YAMLs are updated.

### 🔧 Changed
- **The device's firmware version now matches your charger app.** The Devices page **Firmware** field leads with the version the EVEUS / Grizzl-E app reports as installed (for example `1PGRW001A-R3.05.5`) and appends the secondary module firmware in parentheses (`GRM070A-…`), so nothing is hidden. Previously it showed only the module firmware, which didn't match what the app displays. It still updates on its own after an over-the-air charger firmware update — no reload needed.
- **The drifted-clock warning now tells you the right fix.** Two distinct Repairs messages: a drift sitting at a whole hour raises "Time Zone appears incorrect" (set the **Time Zone** select — pressing Sync Time alone won't help, typical after a daylight-saving change), while any other offset raises the clock-sync message pointing at the **Sync Time** button. A third message covers time zones with a fractional offset (such as India's +5:30): the charger's whole-hour Time Zone select can never match them, so instead of an unresolvable warning you get an explanation of the firmware limitation and how to compensate in charging schedules. If the drift changes character while the warning is shown — say the clock gets synced but the Time Zone is still wrong — the message switches to the matching guidance. Both reference the Time Drift sensor so you can watch the offset return to `0 s`. The warning also now compares local clocks, so a wrong Time Zone or DST mismatch is detected at all — previously it was invisible.
